...
for features=1:15
    Xfeature=X(:,choosefeature(1,7:features+6));
    ...
    MLP_pfea(features,1) = features+6;
end
figure,plot(MLP_pfea(:,1),MLP_pfea(:,2));
MLP_pfeasort=sortrows(MLP_pfea,2);
net1 = newff((X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y',7);
net1.trainParam.goal=0.005;
net1.trainParam.epochs=500;
net1.trainParam.show=20;
net1.trainParam.lr=0.5;
net1.divideParam.trainRatio = 0.8;
net1.divideParam.valRatio = 0.2;
[net1 tr1] = train(net1,(X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y');



netf1 = newff((X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y',7);
...
[netf1 trf1] = train(netf1,(X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y');
%my rbf-bp mixed NN
netf = newff((X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y',[4,4],{'radbas' 'tansig' 'purelin'});
...
[netf trf] = train(netf,(X(:,choosefeature(1,7:MLP_pfeasort(1,1))))',Y');
fit10features = sim(netf,(X(:,choosefeature(1,7:MLP_pfeasort(1,1))))');
fit10features = fit10features';
MLPCresult = sim(netf,(XT(:,choosefeature(1,7:MLP_pfeasort(1,1))))');
