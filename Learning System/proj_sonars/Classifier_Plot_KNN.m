% Classifier Plot
    figure;
    for x = min(InputData(:,1)):(max(InputData(:,1))-min(InputData(:,1)))/50:max(InputData(:,1))
        for y = min(InputData(:,2)):(max(InputData(:,2))-min(InputData(:,2)))/50:max(InputData(:,2))
            Result = ex5_KNN(N,TrainSet,TrainSetOutput,[x y]);
            if Result>0.5
                plot(x,y,'y*');hold on;
            else
                plot(x,y,'c*');hold on;
