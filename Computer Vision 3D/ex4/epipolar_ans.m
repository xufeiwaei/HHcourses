function epipolar_ans(bNormalize)
...
%
% bNormalize: set to 1 to normalize mean and variance
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin == 0
    bNormalize = 0; %normalization, default to off
end
% % Load the images
...
% Image size
[YSIZE, XSIZE] = size(leftim);

hL = figure; imshow(leftim);
hold on;  
hR= figure; imshow(rightim);
hold on;   
...
figure(hL); %put focus on the right image  for displaying clicked points...
for k=1:NPOINTS
text(pl(k,1),pl(k,2),['* ' num2str(k)],'FontSize',10,'color','r'); %OPTIONAL visualize what you clicked
end

figure(hR); %put focus on the right image  for displaying clicked points.... 
for k=1:NPOINTS
text(pr(k,1),pr(k,2),['* ' num2str(k)],'FontSize',10,'color','g'); %OPTIONAL visualize what you clicked
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Verify the epipolar constraint: Draw the the epipolar lines
...
for k = 1:NPOINTS
    ...
    abc=pr(k,:)*F;    %This is obtained by inspecting pr'*F*pl=0, for the point pr(k,:).
    abc=abc/abc(2); %We make sure that the coefficient of y in the line-equation is 1
                    %We assumed that abc(2) is not zero
    y=-abc(1)*x - abc(3);
    plot(x,y,'b'); 
    text(pl(k,1),pl(k,2),['* ' num2str(k)],'FontSize',10,'color','r'); %visualize what you clicked
end

% second image, almost identical procedure, notice the difference in "abc"
figure(hR);
for k = 1:NPOINTS
    abc=F*pl(k,:)';  %This is obtained by inspecting pr'*F*pl=0, for the point pl(k,:).
    abc=abc/abc(2); 
    y=-abc(1)*x - abc(3);
    plot(x,y,'b'); 
    text(pr(k,1),pr(k,2),['* ' num2str(k)],'FontSize',10,'color','g'); 
end
