...
for kk=2:N,
    ...
    Axya = [1 0 -dv*cos(da)*T*sin(A(kk-1)+0.5*dv*sin(da)*T/WHEEL_BASE);
            ...
            0 0 1];
    Au = [cos(da)*T*cos(A(kk-1)+(dv*sin(da)*T)/(2*L))-(dv*cos(da)*T*T*sin(da)*sin(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L) -dv*sin(da)*T*cos(A(kk-1)+(dv*sin(da)*T)/(2*L))-(dv*dv*cos(da)*T*T*cos(da)*sin(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L) cos(da)*dv*cos(A(kk-1)+(dv*sin(da)*T)/(2*L))-(dv*cos(da)*dv*T*sin(da)*sin(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L); 
          cos(da)*T*sin(A(kk-1)+(dv*sin(da)*T)/(2*L))+(dv*cos(da)*T*T*sin(da)*cos(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L) -dv*sin(da)*T*sin(A(kk-1)+(dv*sin(da)*T)/(2*L))+(dv*dv*cos(da)*cos(da)*T*T*cos(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L) cos(da)*dv*sin(A(kk-1)+(dv*sin(da)*T)/(2*L))+(dv*cos(da)*dv*T*sin(da)*cos(A(kk-1)+(dv*sin(da)*T)/(2*L)))/(2*L);
          sin(da)*T/L dv*cos(da)*T/L dv*sin(da)/L];
