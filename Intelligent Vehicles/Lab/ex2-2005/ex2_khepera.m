...
for kk=2:N,
    ...
    dA(kk) = (dDr(kk)-dDl(kk))/WHEEL_BASE; % to get the angle of move
    ...
    X(kk) = X(kk-1) + dX(kk);
    Y(kk) = Y(kk-1) + dY(kk);
    A(kk) = mod(A(kk-1) + dA(kk), 2*pi); %if A(kk-1) + dA(kk) is bigger than 2pi, it means that the vehicle have been move a cycle
