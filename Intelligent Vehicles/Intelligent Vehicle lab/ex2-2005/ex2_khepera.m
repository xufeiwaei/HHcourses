...
for kk=2:N,
    ...
    if dA==0
        ...
        Axya = [
                1 0 -(2*dD*sin(dA/2)*sin(A(kk-1) + dA/2))/dA
                0 1 (2*dD*sin(dA/2)*cos(A(kk-1) + dA/2))/dA
                0 0 1
               ];     % Jacobian matrix with compensation
        Au =   [
                (2*sin(dA/2)*cos(A(kk-1)+dA/2))/dA    (dD*cos(dA/2)*cos(A(kk-1)+dA/2))/dA-(2*dD*sin(dA/2)*cos(A(kk-1)+dA/2))/dA^2-(dD*sin(dA/2)*sin(A(kk-1)+dA/2))/dA
                (2*sin(dA/2)*sin(A(kk-1)+dA/2))/dA    (dD*cos(dA/2)*sin(A(kk-1)+dA/2))/dA+(dD*sin(dA/2)*cos(A(kk-1)+dA/2))/dA-(2*dD*sin(dA/2)*sin(A(kk-1)+dA/2))/dA^2
                0                                     1
               ];           % Jacobian matrix with compensation
    end
