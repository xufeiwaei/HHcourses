...
syms fx fy fv fa fA ft fl

f = [fx + fv*cos(fa)*ft*cos(fA+fa*sin(fa)*ft/fl)
    fy + fv*cos(fa)*ft*sin(fA+fa*sin(fa)*ft/fl) 
    fA + fv*sin(fa)*ft/fl];
...
for kk=2:N,
    ...
    Axya = subs(jacobian(f, [fx, fy, fA]), {fv fa fA ft fl}, {dv, da, A(kk-1), T, WHEEL_BASE});
    
    %Axya = [1 0 -dv*cos(da)*T*sin(A(kk-1)+0.5*dv*sin(da)*T/WHEEL_BASE);
    %        0 1 dv*cos(da)*T*cos(A(kk-1)+ 0.5*dv*sin(da)*T/WHEEL_BASE);
    %        0 0 1];
    
    Au = subs(jacobian(f, [fv, fa, ft]), {fv fa fA ft fl}, {dv, da, A(kk-1), T, WHEEL_BASE});
