...
for kk=2:N,
    ...
    % Predict the new uncertainty in the state variables (Error prediction)
    Cxya_old = [P(kk-1,1:3);P(kk-1,4:6);P(kk-1,7:9)];   % Uncertainty in state variables at time k-1 [3x3]    
    Cu = [dDr(kk) 0;0 dDl(kk)];

    Cb = (53-45)/53;
    Ab = []
        
    Axya = [1 0 (-(dDr+dDl)/2)*sin(A(kk-1)+(dDr-dDl)/2);
            0 1 ((dDr+dDl)/2)*cos(A(kk-1)+(dDr-dDl)/2);
            0 0 1];
    Au = [cos(A(kk-1)+(dDr-dDl)/2) (-(dDr+dDl)/2)*sin(A(kk-1)+(dDr-dDl)/2);
          sin(A(kk-1)+(dDr-dDl)/2) 0.5*((dDr+dDl)/2)*cos(A(kk-1)+(dDr-dDl)/2);
          0 1];
