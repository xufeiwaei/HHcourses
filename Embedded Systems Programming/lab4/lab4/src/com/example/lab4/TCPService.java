...
public class TCPService extends Service{
	...
	public void handleMessage(Message msg) {
		...
		else if(c=='e')
		{
			message_state = 2;
			Record += back;
			sendNotification("The teacher have received the answer! Show you later!");
		}
		else
		{
			message_state = 3;
	...
	private void sendNotification(String s){
	  ...
	  //@@ essa send notification if the string is form the server ONLY.
	   // If it is form client that is  if String S  is  "Sent: " + data + ", Got: " + back + "." you don't have to sent notification
	    if(message_state == 2)
	    {
	    	tickerText = "Answer received";              // ticker-text
			contentTitle = "Answer received";  // message title
			contentText = "The teacher have received the answer! Show you later!";      // message text
	    }
	    else if(message_state == 1)
	    {
	    	tickerText = "NewQuestion";              // ticker-text
			contentTitle = "Question is arrived!";  // message title
			contentText = "Answer the question!";      // message text
			QuestionNum ++;
			Record += s;
	    }
	    else
